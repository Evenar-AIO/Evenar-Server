...
exports.processPayment = async (payload) => {
  const { orderId, method, returnUrl: customReturn, cancelUrl: customCancel } = payload;
  ...
  if (method === 'PAYOS') {
    ...
    // Link mobile deep linking support or proxy redirect if provided
    const returnUrl = customReturn || `${domain}/checkout?status=success&orderId=${order._id}`;
    const cancelUrl = customCancel || `${domain}/checkout?status=cancelled&orderId=${order._id}`;
